/*
EJ11.01 - Crear una escena con un suelo, varios cubos de madera (buscar textura)
y varios trozos de hierba delante de los cubos con la textura adjunta, usando blending.
*/
...
void render(const Geometry& quad, const Geometry& object, const Shader& s_phong, const Shader& s_blend,
    const Texture& t_albedo, const Texture& t_specular, const Texture& t_albedo_wood, const Texture& t_specular_wood,  const Texture& t_grass) {
    ...
    for (const auto& cubePos : cubePositions)
    {
        for (int i = 0; i < 3; i++) {
            s_blend.use();
            model = glm::mat4(1.0f);
            model = glm::translate(model, glm::vec3(cubePos.x - 0.5f + 0.5f*i, cubePos.y, cubePos.z + 1.0f+0.01*i));
...
int main(int, char* []) {
    ...
    while (window->alive()) {
        ...
        render(quad, cube, s_phong, s_blend, t_albedo, t_specular, t_albedo_wood, t_specular_wood, t_grass);
        window->frame();
    }
