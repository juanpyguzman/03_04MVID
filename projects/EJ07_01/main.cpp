...
void render(const Geometry& object, const Geometry& light, const Shader& s_blinn, const Shader& s_light) {
    ...
    //A partir de la posición inicial de la luz, la vamos girando en los ejes "x" y "z" describiendo un círculo
    glm::vec3 lightPosNew = glm::vec3(lightPos.x * sin(static_cast<float>(glfwGetTime())), lightPos.y, lightPos.z * cos(static_cast<float>(glfwGetTime())));
    ...
    s_blinn.set("ambientStrength", 0.2f);
    s_blinn.set("lightPos", lightPosNew);
...
int main(int, char* []) {
    ...
    const Shader s_blinn("../projects/EJ07_01/phong.vs", "../projects/EJ07_01/blinn.fs");
