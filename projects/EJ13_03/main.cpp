/*
EJ13.03 - Usando una escena 3D cualquiera, implementar el postproceso de Blur Gaussiano
en dos pasadas (https://en.wikipedia.org/wiki/Gaussian_blur  ->  seccion implementacion).

Se ha usado la implementación del siguiente blog:
...
void render(const Geometry& quad, const Geometry& cube, const Shader& s_phong, const Shader& s_blur_hor, const Shader& s_blur_ver,
    ...
    s_phong.set("light.ambient", 0.2f, 0.0f, 0.0f);
    s_phong.set("light.diffuse", 0.8f, 0.8f, 0.8f);
    s_phong.set("light.specular", 0.0f, 0.0f, 0.0f);
    ...
    //SECOND PASS (Horizontal pass for Blur)
    ...
    s_blur_hor.set("screenTexture", 0);

    quad.render();

    //THIRD PASS (Vertical pass for Blur)
    ...
    s_blur_ver.set("screenTexture", 0);
...
int main(int, char* []) {
    ...
    const Shader s_blur_hor("../projects/EJ13_03/blur.vs", "../projects/EJ13_03/blur_hor.fs");
    const Shader s_blur_ver("../projects/EJ13_03/blur.vs", "../projects/EJ13_03/blur_ver.fs");
    ...
    while (window->alive()) {
        ...
        render(quad, cube, s_phong, s_blur_hor, s_blur_ver,
            t_albedo, t_specular,
            std::get<0>(fbo1), std::get<1>(fbo1), std::get<0>(fbo2), std::get<1>(fbo2));
        window->frame();
    }

    glDeleteFramebuffers(1, &std::get<0>(fbo1));
    glDeleteTextures(1, &std::get<1>(fbo1));
    glDeleteRenderbuffers(1, &std::get<2>(fbo1));

    glDeleteFramebuffers(1, &std::get<0>(fbo2));
    glDeleteTextures(1, &std::get<1>(fbo2));
    glDeleteRenderbuffers(1, &std::get<2>(fbo2));
