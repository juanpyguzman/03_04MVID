/*
Con la escena anterior, añadir dos quads solidos, uno más cercano que el resto,
y otro entre los quads de colores.  De manera que los quads de color actuen correctamente como un cristal de color.
*/
...
//Posición de los Quads sólidos
glm::vec3 quadDiffusePositions[] = {
...
void render(const Geometry& quad, const Shader& s_phong, const Shader& s_blend,
    ...
    //Quad sólido
    for (const auto& quadDiffusePos : quadDiffusePositions) {
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(quadDiffusePos));
        s_phong.set("model", model);
        s_phong.set("view", view);
        s_phong.set("proj", proj);
...
int main(int, char* []) {
    ...
    const Shader s_blend("../projects/EJ11_04/blend.vs", "../projects/EJ11_04/blend.fs");
