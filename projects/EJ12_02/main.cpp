/*
EJ12.02 - Usar el importador de modelos para crear una escena con algún modelo complejo
y pintarlo usando normal mapping con el modelo de iluminación Blinn.
*/
...
void render(const Geometry& floor, const Model& object, const Shader& s_normal, const Shader& s_phong,
    ...
    lightPos = glm::vec3(2.0f*std::sin((float)glfwGetTime() / 4.0), 0.3f, 2.0f*std::cos((float)glfwGetTime() / 4.0));
    ...
    //Modelo 3D con normal mapping (a la izquierda)
    {
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(-1.0f, -1.0f, -0.2f));
    ...
    //Modelo 3D sin normal mapping (a la derecha)
    {
        s_phong.use();
...
int main(int, char* []) {
    ...
    const Quad quad(2.0f, 4);
    const Model model("../assets/models/Freighter/Freigther_BI_Export.obj");
    ...
    while (window->alive()) {
        ...
        render(quad, model, s_normal, s_phong, t_albedo, t_specular, t_normal);
        window->frame();
    }
